...
(function () {
    ...
    if (
      ...
      if (url.includes("you/likes")) {
        btnShuffle.className =
          "bhackel-shuffle-likes sc-button sc-button-large";
      ...
      // Case for generic user likes
        ...
        btnShuffle.className =
          "bhackel-shuffle-likes sc-button sc-button-medium";
      ...
      // Case for discover playlists
        ...
        // Check if action bar has loaded
        let playlistControls = document.querySelector(
          ".systemPlaylistDetails__controls"
        );
        if (playlistControls) {
          // Insert the button after other action buttons
  ...
  function setupLoad(btn) {
    ...
    if (btn.scrolling === false) {
      ...
      if (tracks.childElementCount > 2) {
        ...
        document
          .getElementsByClassName("moreActions__button addToNextUp")[0]
          .click();
  ...
  function toggleQueue(changeToState) {
    ...
    if (
      (isQueueOpen && changeToState === "close") ||
      (!isQueueOpen && changeToState === "open")
    ) {
      let queueTrigger = document.querySelector(
...
(function () {
  ...
  const pressMoreButton = async (button) => {
    console.log({ button });
    button.click();
    setTimeout(() => {
      let btnAddToPlaylist = document.querySelector(".sc-button-addtoset");

      if (btnAddToPlaylist) {
        console.log("Button to add song: ", btnAddToPlaylist);
        btnAddToPlaylist.click();
        setTimeout(() => {
          let ulList = document.querySelector(".addToPlaylistList__list");
          console.log({ ulList });
          console.log(ulList.children);

          if (ulList) {
            setTimeout(() => {
              let children = ulList.children[0].children;
              console.log({ children });

              for (let i = 0; i < children.length; i++) {
                if (children[i].innerHTML.includes('title="Test Playlist"')) {
                  console.log("This is the one: ", i);
                  console.log(children[i]);
                  let button2 = children[i].children[0].children[2].children[1];
                  if (button2) {
                    if (button2.innerHTML === "Added") {
                      closeModal();
                    } else {
        ...
        console.log("No Button to Add to playlist");
      }
    }, 2000);
  };
  ...
  const findButton = async () => {
    let btnMore = document.querySelectorAll(".sc-button-more");
    let play = document.querySelectorAll(".playButton");
    console.log({ play });
    console.log({ btnMore });
    await delay(2000);
    ...
    if (btnMore) {
      for (let i = 0; i < 2; i++) {
        console.log(btnMore[i]);
        console.log(await pressMoreButton(btnMore[i]));
        await delay(5000);
      }
    } else {
